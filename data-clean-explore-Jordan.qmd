...
REALIZE AN ISSUE WITH LATITUDE AND LONG 
...
Need to see how many species are found in `biodiv_total` and `range_edge_category`
```{r}
...
Notes on how we can improve apon range categories 

- Make a column that sums the present observations 
- Make a column that sumns the absent observations 
