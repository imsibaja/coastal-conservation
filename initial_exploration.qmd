...
dangermond_range_edge <- biodiv_total %>%
  ...
  summarize(
    present_below_south = any(latitude < PC_south_bound & presence==1),
    present_within_buffer = any(latitude >= PC_south_bound & presence == 1) &
        any(latitude <= PC_north_bound & presence == 1),
    present_above_north = any(latitude > PC_north_bound & presence==1),
    
    # Observations where Present
    present_observations = sum(presence == 1, na.rm = TRUE),
    
    # Observations Present/Absence
    total_observations = n(),
    
    # distinct sites where present
    sites = n_distinct(marine_site_name[presence==1]),
    
    # Total Counts
    total_counts = sum(num_count),
    
    # Fraction of years present
    percent_years_present = n_distinct(year[presence==1]) / n_distinct(year) * 100
  ) %>%
  mutate(
...
tm_shape(CA_coastline_shp) +
  tm_lines(col = "#bae4b3") + # land color 
tm_shape(dangermond) +
  tm_polygons(col = "orange") +
tm_shape(coastline) +
    tm_dots(col = "red", size = 0.1) +
