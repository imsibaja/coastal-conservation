...
PC_north_bound <- 34.9331449422735
PC_south_bound <- 34.4727142766036
...
dangermond_range_edge <- biodiv_total %>%
    group_by(species_lump) %>%
    summarize(
        present_below_south = any(latitude < PC_south_bound &
                                      presence == 1),
        
        present_within_buffer = any(
            latitude >= PC_south_bound &
                latitude <= PC_north_bound & presence == 1,
            na.rm = TRUE
        ),
        
        present_above_north = any(latitude > PC_north_bound &
                                      presence == 1),
        
        
        # Observations where Present
        present_observations_site = sum(
            latitude >= PC_south_bound &
                latitude <= PC_north_bound &
                presence == 1,
            na.rm = TRUE
        ),
        ...
        sites_in_buffer = n_distinct(marine_site_name[latitude >= PC_south_bound &
                                                          latitude <= PC_north_bound], na.rm = TRUE),
        
        
        # distinct sites where species present within buffer
        sites_present = n_distinct(marine_site_name[latitude >= PC_south_bound &
                                                        latitude <= PC_north_bound &
                                                        presence == 1], na.rm = TRUE),
        ...
        percent_years_present = n_distinct(year[latitude >= PC_south_bound &
                                                    latitude <= PC_north_bound &
                                                    presence == 1], na.rm = TRUE) / n_distinct(year, na.rm = TRUE) * 100
    ...
    mutate(
        ...
        range_edge_category = case_when(
            present_above_north &
                present_within_buffer &
                !present_below_south ~ "Southern Range Edge",
            !present_above_north &
                present_within_buffer &
                present_below_south ~ "Northern Range Edge",
            present_above_north &
                present_within_buffer &
                present_below_south ~ "Continuous Presence",
            present_above_north &
                !present_within_buffer &
                present_below_south ~ "Avoidant",
            !present_above_north &
                !present_within_buffer &
                present_below_south ~ "Southern Only Presence",
            present_above_north &
                !present_within_buffer &
                !present_below_south ~ "Northern Only Presence",
            !present_above_north &
                present_within_buffer &
                !present_below_south ~ "Dangermond Endemic Presence",
            TRUE ~ "Absent"
        )
    ) %>%
    ungroup() 
...
```{r}
# Create a sf object for the buffer lines
buffer_lines <- st_sfc(
  st_linestring(rbind(c(-125, PC_south_bound), c(-115, PC_south_bound))), # north bound
  st_linestring(rbind(c(-125, PC_north_bound), c(-115, PC_north_bound)))  # south bound
) %>% 
...
coastline <- read_csv(here::here("data", "coastal_spatial_data",
                                 "coast.CA.100km.chunks.csv"))

coastline_lat <- coastline$Y %>% sort(decreasing = FALSE)
...
```{r}
coastline <- coastline %>% 
  st_as_sf(coords = c("X", "Y"), crs = st_crs(dangermond), remove = FALSE)

# Check that the crs matches 
if(st_crs(coastline) == st_crs(biodiv_geo)) {
  print("The coordinate reference systems match")
} else {
  print("The coordinate reference systems do NOT match. Transformation of CRS is recommended.")
}
...
CA_coastline_shp <- read_sf(here('data', 'coastal_spatial_data',
                                 'coast.line.CA.shp')) #%>%
    #st_transform(crs=biodiv_geo)

tm_shape(CA_coastline_shp) +
  tm_lines(col = "#000000") + # land color 
tm_shape(california) +
  tm_polygons(col = "#bae4b3") + # land color 
tm_shape(dangermond) +
  tm_polygons(col = "orange") +
tm_shape(biodiv_geo) +
    tm_symbols(col = "blue", size = 0.05, shape = 20) + # Add buffer lines
tm_shape(coastline) +
    tm_symbols(col = "red", size = 0.1, shape = 4) +
tm_add_legend(type = "symbol",  
              labels = c("MARINe Site", "100km chunk") , 
              col = c("blue", "red"),
              shape = c(20, 4)) + 
tm_scale_bar(position = c(0.02, 0.02), width = 0.2) + # scale bar
tm_compass(position = c(0.01, 0.08), text.size = 0.8) + # compass
tm_layout(bg.color = "gray82", # background ocean color 
...
# Define a polygon that will cut off SF Bay
golden_gate_line <- st_linestring(matrix(c(-122.513, 37.826,  # Marin Headlands
                                           -122.477, 37.807), # San Francisco Side
                                         ncol = 2, byrow = TRUE)) %>%
  st_sfc(crs = st_crs(coastline)) %>%
  st_buffer(0.01)

# Define SF Bay bounding box (approximate)
sf_bay_bbox <- st_bbox(c(xmin =  -121, xmax = -122.513, 
                          ymin = 37.807, ymax = 37.826), 
                        crs = st_crs(CA_coastline_shp)) %>%
  st_as_sfc()

# Remove the SF Bay portion from the coastline
coastline_trimmed <- st_difference(CA_coastline_shp, sf_bay_bbox)

# Plot the modified coastline
tm_shape(CA_coastline_shp, bbox = sf_bay_bbox) + 
    tm_lines() +
tm_shape(sf_bay_bbox) +
    tm_borders(col = "red") +
tm_shape(golden_gate_line) +
    tm_borders()
